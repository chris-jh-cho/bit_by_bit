...
class SpreadBasedMarketMakerAgent(TradingAgent):
    ...
    def receiveMessage(self, currentTime, msg):
        """ 
        Processes message from exchange. Main function is to update orders
        in orderbook relative to mid-price.
        ...
        # this condition is necessary to prevent giving empty "mid" inputs to computeOrdersToCancel
        else:
            return
    ...
    def computeOrdersToCancel(self, mid="error"):
        """ 
        Given a mid price, computes the orders that need to be removed from
        orderbook, and pops these orders from bid and ask deques.
        ...
        """
        2021/01/04 Chris Cho
        
        "mid" can be empty according to the current architecture. This will 
        throw an error, before this function can return an empty array of 
        orders to cancel. Therefore, we have set the default value of "mid" to
        a string. This will trigger an obvious error if anything else is going
        wrong.
        """
    ...
    def cancelOrders(self, orders_to_cancel):
        """ 
        Given a list of _Order objects, remove the corresponding orders from 
        ExchangeAgent's orderbook
    ...
    def computeOrdersToPlace(self, mid):
        """ Given a mid price, computes the orders that need to be removed
            from orderbook, and adds these orders to bid and ask deques.
    ...
    def placeOrders(self, mid):
        """ 
        Given a mid-price, compute new orders that need to be placed, then send
        the orders to the Exchange.
    ...
    # 20210513 Chris Cho: function to query order size
